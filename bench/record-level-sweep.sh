...
# `RECORD_LEVEL` (`vendor/redoxfs/src/lib.rs`). Nothing in `filesystem_protocol` can ask for a different one, so
...
# - It cannot sweep the TRANSFER size, only the record level, because a `filesystem_protocol` request carries at
