# scripts/queue-eligible.jq: what "eligible for the merge queue" means, in one place.
#
...
# of theirs: jq -r "$(cat scripts/queue-eligible.jq)" '[ .[] | eligible | ... ]'. A missing file
# leaves `eligible` undefined, jq refuses the program, and the consumer's `|| echo '[]'` arms
# nothing, which is the direction to fail in. scripts/queue-eligible-selftest.sh checks the
