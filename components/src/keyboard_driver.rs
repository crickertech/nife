...
//! never by the driver's own choice, which is the same authority `components/src/input.rs` has always had
...
pub extern "C" fn _start(mode: u64, dma_phys: u64, _arg2: u64) -> ! {
    ...
    loop {
        ...
        if mode == MODE_DIRECT {
            ...
            // rule `components/src/input.rs`'s own `drain` follows for the UART side.
            direct_send(&direct_buf, &mut direct_n);
        } else if typed {
            ...
            // rescans every client's control page before it composites (see components/src/display_terminal.rs).
            ring_publish(tail);
