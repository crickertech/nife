...
//! about what to say is `multicast_dns_protocol`'s** (host-tested against real bytes from the reference router,
//! Kani-checked at the parser), **everything it says is `multicast_dns_config`'s** (a document a person
...
//!    `socket_protocol::udp_bind_grant`, and a responder spawned without it is refused (see
//!    notes/mdns.md).
...
//!    is one `multicast_dns_protocol::respond` call: multicast queries are answered to the group, legacy
...
//! # Socket ids
//!
//! Sid 4, not 0. A stack endpoint's socket table is per-endpoint (`socket_protocol`'s BUGS: clients
...
//! # BUGS
//!
//! - **The configuration is compiled in, not read from a file.** `multicast_dns_config`'s document is
...
//!   gate cannot seed today, and the fix is a `FileSpec` grant plus an `filesystem_protocol` open-and-read at
...
//!   `multicast_dns_protocol` has the wire halves (`probe_query`, `conflicts`, `tiebreak`) ready. This
...
//!   MTU would drop. RFC 6762's answer is the TC bit and a follow-up message, which `multicast_dns_protocol`
...
//! - **IPv4 only.** No AAAA, so a Mac on an IPv6-only segment finds nothing. `multicast_dns_protocol`'s
...
use abi::rights;
use multicast_dns_config::Config;
use multicast_dns_protocol::{GROUP_V4, PORT, QuerySource, Service, announcement, respond};
use socket_protocol::{
...
/// **The advertisement, as a document rather than as constants.** See `multicast_dns_config`: the values are
/// measured off a working Time Machine server, and they are data every step of the way from that
/// file to the wire.
const CONFIG: &str = include_str!("../multicast_dns_responder.conf");
...
/// transport's MTU cannot carry the whole record set (see [`multicast_dns_protocol::announcement`]).
fn announce(adv: &multicast_dns_protocol::Advertisement<'_>) {
...
fn serve_one(adv: &multicast_dns_protocol::Advertisement<'_>) -> Received {
    ...
    // `multicast_dns_protocol` applies every one of those rules; this program's whole part in it is knowing
...
pub extern "C" fn _start(rounds: u64, ipv4: u64, _a2: u64) -> ! {
    ...
    match call(STACK, req(OP_BIND_UDP, SID), PORT as u64).0 {
        ...
        // authority is the spawn site's to grant (`socket_protocol::udp_bind_grant`).
...
fn config_error_line(e: multicast_dns_config::Error) -> usize {
    use multicast_dns_config::Error::*;
    match e {
