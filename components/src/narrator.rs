...
//! `memory_grant_depleter` already are, and it names what the program does rather than what it holds, which is
...
//! endpoints, `fixtures/src/hello.rs`'s `printing_client` shape), and `swish` starts a program with an
...
/// `components/src/console.rs`, which is the other end.
