...
/// the window.
fn sink_producer() -> ! {
    let payload = [b'x'; byte_sink_protocol::INLINE_MAX];
    for _ in 0..(SINK_MSGS + 64) {
        let (w0, w1, w2, _) = byte_sink_protocol::pack(&payload);
        send(SINK_PIPE_OUT, w0, w1, w2);
    }
    send(SINK_PIPE_OUT, byte_sink_protocol::eof(), 0, 0);
    exit();
}
...
fn sink_consumer() -> ! {
    ...
    send(
        REPORT,
        ticks,
        SINK_MSGS * byte_sink_protocol::INLINE_MAX as u64,
        0,
    );
    exit();
}
