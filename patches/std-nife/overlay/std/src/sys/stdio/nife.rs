//! stdout/stderr over the **sink contract** (`crates/byte_sink_protocol`, milestone 50), on the endpoint
...
//! must end when `head` exits. `byte_sink_protocol::classify` splits the two, and each half is routed
//! through the seam std already has for exactly this:
//!
//! - **No sink** ([`byte_sink_protocol::Sent::NoSink`]) is [`io::ErrorKind::Unsupported`], which
...
//! - **A destroyed sink** ([`byte_sink_protocol::Sent::Gone`]) is [`io::ErrorKind::BrokenPipe`], which
