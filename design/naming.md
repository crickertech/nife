...
- **`helpers/`** is the helper drawer: `.sh` extension, called by `script/` entry points and by
  `xtask`, not by people (`qemu-bounded.sh`, `qemu-runner-aarch64.sh`, `qemu-runner-riscv64.sh`).
  **It was `scripts/` until 2026-09-23**, when calef ratified `helpers` after opening the tree and
  asking *"why do we have script and scripts?"*. The split between a front door and the plumbing
  behind it is sound and did not change; one trailing letter was never able to carry it, so the only
  thing telling the two apart was knowing. Records that describe the past keep the old name. See
  [scripts.md](../notes/scripts.md).
...
- **The tree has more kinds of name than the table covers, and this bullet is the one place that
  ...
  entry points and Cargo packages carry blocks. Directories, types and the `helpers/` drawer
...
- **The `helpers/` drawer is deliberately outside the worklist, and the numbers are why.** There were
  17 files in it when this was priced on 2026-09-20 (as `scripts/`, its name until 2026-09-23), of which **9 already carry a `Name:` paragraph** that nobody asked them
