# Boot to a login prompt, into `swish`, to editing a file with `kilo`
...
does not wait on step 3; step 4's blocker is a device grant chain unrelated to either).

## What "done" looks like

Every step above at `BUILT`, with step 4's cited decision resolved in the direction that lets login
actually run (either milestone 159 landing, or a revisit of §120). At that point the story in the
title is something a person can actually do, not something proven in five separate test harnesses.
