...
- **Unclaimed.** Take the 32-bit MMIO window that BARs are placed in from the machine rather than
  from the hardcoded `arch::x86_64::mmu::PCI_BAR_PHYS`. `_CRS` is AML and stays refused, but the
