...
- **Recorded.** `design/naming.md` lists what this milestone deliberately did not rename, so the next
...
- **Recorded.** `design/naming.md` also keeps this block's `target/` and `targets/` entry: build
