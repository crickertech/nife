...
**Status: PROPOSED 2026-09-20.** Filed by milestone 447's lane, which built the thing that was
blocking the flip and deliberately did not take it.
...
in milestone 184's own words, because `kernel/src/arch/x86_64/` saved no FPU or SSE state on a
...
category, and [DECISIONS §22](../../decisions/22-rust-std-on-the-native-abi.md) picked the current
...
- **One flag disappears.** `--cfg aes_force_soft`, in `.cargo/config.toml`'s
  ...
  other. Milestone 164's block and the framing this proposal inherited both suggest a family of
...
- **Possibly [milestone 442](../442-a-crypto-provider-that-builds-on-bare-metal.md)'s second
  failure class**, and this is the one worth measuring first; see below.
...
1. **Re-measure milestone 442's soft-float failures against nife's own targets.** 442's block
