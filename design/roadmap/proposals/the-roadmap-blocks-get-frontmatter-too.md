...
**Status: PROPOSED 2026-09-23.** Raised by the lane of milestone 581 (a decision's status becomes a
field, and the index becomes generated), which migrated `design/decisions/` and was briefed to
propose this rather than do it.

**Gate: MILESTONE 581.** Not because the code depends on it, but because 581 is the pilot and this
...
`design/roadmap/` carries the same defect `design/decisions/` carried until 581. A block's status
...
The migration would be the same four steps 581 took: a schema, a mechanical rewrite of every block,
a script that reads fields instead of sentences, and a rule that the prose may not restate a field.

## Why it is not simply 581 again
...
One lane, on the evidence of 581, where the mechanical rewrite was the cheap part and the
provenance archaeology was the expensive one. The roadmap is younger and better dated than the
decisions corpus, so the equivalent of 581's 152 missing raise dates may be much smaller. Nobody
...
No number. The integrator mints one at promotion, and 581's lane was told not to reach for it.
