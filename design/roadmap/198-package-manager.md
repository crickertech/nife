...
the run. **Rung 3a's consumer half is BUILT through "verified by digest" (2026-09-24,
`milestone/198-rung-3a-consumer`)**: the image carries its own package catalogue above the
measurement table, a host process serves the package over plain HTTP, and `net_stack`'s client
...
Added 2026-09-23 by the lane that built rung 3a's producer half and extended 2026-09-24 by the lane
that built fetch and verify, and covering rung 3a only: this
...
- **Proposed.** The booted system has no network: the progenitor builds no `net_stack`, so the
  fetch runs only in the kernel's test harness.
  `design/roadmap/proposals/the-booted-system-has-no-network.md`.
- **Done.** A gate runs `cargo xtask package` end to end: every archive build runs every recipe for
  its architecture since 2026-09-24, and the package tests fetch what it produced.
...
- **Recorded.** No compression, a `u32` ceiling on a member and on a package, a catalogue that is
  one file in `target/` and one archive entry rather than a repository index, a fetch that runs only
  in the test harness and not on x86_64, a package peer that is slirp's `guestfwd` rather than a
  host on a real network, and a recipe that cannot say where its
...
- ~~**Nothing gates the producer end to end in CI.**~~ The archive build runs it since 2026-09-24,
  so every gate that builds an image does.
