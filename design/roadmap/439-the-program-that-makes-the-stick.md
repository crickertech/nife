# 439. The program that makes the stick: one download per host, a boot for every architecture
...
|---|---|---|
| `stick_maker` (provisional), the host program | `crates/stick_maker` | host tests on captured `diskutil` documents; `scripts/stick-maker-proof.sh` on macOS; `.github/workflows/stick-maker-hosts.yml` on Linux and Windows runners |
