...
root-level workspaces or `scripts/`". calef asked for a lane to close that hole. The maintainer ran
...
| `script/names`' own scope comment | `kernel`, `xtask`, `redoxfs_server`, `tools/` and `scripts/` are "out of scope on purpose" | four of those five are covered; only `scripts/` is out |
...
found through, and was deliberately left alone. `scripts/name_provenance.py` and
`scripts/roadmap_proposals.py` each say they are out of the worklist's scope, which is still true,
and each says so while carrying its own `Name:` paragraph as the record instead, which is the
convention working.

## `scripts/` stays out, and here is the price of the alternative

The one genuinely uncovered surface besides types is `scripts/`, the helper drawer, as distinct
from `script/`'s entry points. It was priced rather than assumed, and refused. The numbers:

- **17 files** in `scripts/`, plus the `kani-lint-shim/` directory, which is Rust source rather
  than a script.
...
- **Zero of the 9 paragraphs records a refusal.** The claim of milestone 115 (the names that were
  ...
  that half is empty here, so leaving `scripts/` out loses nothing that mechanism was built to
...
does not need coverage to fix: the tool now recognises a `scripts/<name>.sh` or `scripts/<name>.py`,
...
mention of it (`script/names`' comment, the two `scripts/` python modules, `notes/scripts.md`)
...
- **Types are still uncovered and were not priced.** `design/naming.md`'s `BUGS` names types
  alongside `scripts/` as the surfaces without blocks. This milestone measured `scripts/` because
...
- **The `scripts/` refusal is keyed on a number that will move.** Zero of the nine paragraphs
  records a refusal today. The first `scripts/` helper whose name is argued against a rival that
...
- **Refused.** Enumerating `scripts/` helpers in the naming worklist, priced above at ~15 rows on a
...
- **Recorded.** Types carry no provenance blocks and the surface was never counted, beside the
  `scripts/` refusal in `design/naming.md`'s `BUGS` section.
...
somebody to build a thing that exists. The genuinely uncovered surface, `scripts/`, was priced
...
and `design/naming.md` defines `scripts/` as the drawer people do not type. What the refusal still
