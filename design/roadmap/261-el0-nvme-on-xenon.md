...
**Done 2026-09-17**, per `design/fatal-risks.md` risk 6 ("calef ran that wipe on 2026-09-17").
Corrected 2026-09-24: this line still read "not done" a week after it happened.
...
The remaining step is now one boot image and one page. `cargo xtask disk-throughput --stage-only`
stages a kernel built with the `disk_throughput` feature (provisional) at
`target/esp-disk-throughput`; it runs the tour, prints **two preflight verdicts before it measures
anything**, times a verified 64 MiB write and read through the EL0 server, and ends in one verdict
line: `CONFINED-AT-RATE`, `UNCONFINED`, `SKIPPED` or `FAILED`. The procedure, the expected output
and what each outcome means were written before any number exists:
[notes/risk-6-bench-evening.md](../../notes/risk-6-bench-evening.md).

**The rehearsal found the load-bearing unknown below was worse than stated.** The kernel brought up
the DMAR's first unit, and on a client Intel machine that is the integrated GPU's: the Skylake
OptiPlex 7040 publishes `0xfed90000` flags `0x0` then `0xfed91000` flags `0x1`, and xenon's first
unit is `0xfed90000` in a 204-byte DMAR. And `confined_by_iommu` was `iommu::is_active()`, true
whenever *any* unit translates. So the NVMe test would most likely have passed on xenon about a
device nothing was translating. Now the kernel decodes every scope (`machine_discovery::acpi::
DmarUnits`), brings up the catch-all unit, and asks the DMAR who owns the requester
(`iommu::scope_of`); the test asserts ownership, and fails rather than skips on a controller the
driver refuses. Rehearsed under OVMF in four machine shapes, two of them built to come out red.
...
- **Whether xenon's DMAR gives the NVMe to the unit this kernel translates is still unread**, and
  it is still the load-bearing unknown: with the driver at EL0 the IOMMU is the whole confinement
  story. What changed on 2026-09-24 is that the machine now answers it in print. The bench boot's
  `preflight 1/2 dmar scope` line names the owning unit and how, the tour lists every unit, and a
  FAIL labels the measurement `UNCONFINED` rather than letting it stand. The catch-all path has run
  against host-test tables only, because QEMU presents one unit; the first xenon boot is its first
  real input.
...
- **Outstanding.** The bench step, which is the whole of what remains and is the only part that
  answers fatal risk 6: three boots of `cargo xtask disk-throughput --stage-only`'s image on xenon,
  photographed, read by [notes/risk-6-bench-evening.md](../../notes/risk-6-bench-evening.md). The
  disk is wiped. Checked 2026-09-24.
- **Outstanding.** Answered by that same boot: whether xenon's DMAR scope covers the NVMe
  function is the bench boot's first preflight line. Checked 2026-09-24.
- **Proposed.** `design/roadmap/proposals/every-vt-d-unit-translates-its-own-devices.md`: bring up
  every VT-d unit and route each device to its owner.
  Today exactly one unit translates, the catch-all. If xenon's preflight says the NVMe belongs to a
  unit that is not up, this is the fix; if it passes, this is still what a machine with more than
  one non-graphics unit needs. It also wants RMRR identity maps, which nothing here builds.
- **Proposed.** `design/roadmap/proposals/the-nvme-test-on-a-machine-whose-iommu-does-not-own-it.md`:
  the NVMe boot test on a machine whose IOMMU does not own the controller. `cargo xtask disk-throughput --case bypass` shows `-machine
  default_bus_bypass_iommu=on` builds exactly that machine under OVMF. Running the ordinary suite's
  NVMe case on it and expecting red is the replayable falsification of the `confined_by_iommu`
  assertion, which before 2026-09-24 would have stayed green.
