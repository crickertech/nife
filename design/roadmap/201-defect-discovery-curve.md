...
bugs (milestone 161) as the curve's first three data points.
...
`BUGS` #3) was fixed by milestone 316 (2026-09-17), and the third-or-later-secondary race (`BUGS` #1)
was fixed by milestone 161's own closing work (2026-09-19). A third x86_64 SMP bug from the same
...
2. **x86_64: a boot-core-identity mixup** (`ap_boot`'s `BUGS` #3). `boot_cpu_id` answered "which core
   am I" instead of "which core booted," so a test body DECISIONS §28's placement migrated onto a
   secondary mistook that secondary for the boot core. Found 2026-08-25 (while verifying #2 above);
   fixed by milestone 316, 2026-09-17.
