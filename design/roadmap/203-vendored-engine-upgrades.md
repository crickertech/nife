...
- **A bump is where the divergences get re-litigated**, and divergence 3 in particular is one
  `vendor/README.md` says is kept rather than reverted only because reverting it is an architect's call.
...
- **Recorded.** A bump is where the five divergences get re-litigated, and divergence 3 is kept
  rather than reverted only because reverting it is an architect's call. Three of the five are re-applied
