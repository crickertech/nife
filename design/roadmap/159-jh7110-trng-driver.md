...
(`components/src/jh7110_entropy_source.rs`). The second lane (2026-09-01) wired that program to a spawner, gave it
...
- **A driver, not a new protocol.** `entropy_service`'s own contract with its clients does not
  ...
  base address and knows nothing else. `components/src/jh7110_entropy_source.rs` is that backend, speaking
...
`entropy_proto`, in `components/src/entropy.rs` and in `components/src/jh7110_entropy_source.rs` rather than left implicit.
...
- **Recorded.** The 128-bit-mode question stays a `BUGS` entry in `components/src/jh7110_entropy_source.rs` until a
