# 193. Sessions interleave rather than serialize, and a renumber is the price
...
A 333-file branch (milestone 433's drain, pull request #970) took three merge conflicts and three
renumbers of `design/decisions/` in ninety minutes, because a second session was minting sections
from the same range and landing every ten minutes. It minted §156, then §157, then §158; each
landing displaced the first branch's whole run by one, from §156-§189 to §159-§192.
...
three renumbers of 2026-09-19 were done, the third by a script rather than by hand.
...
fired three times in ninety minutes.
