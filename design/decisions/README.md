...
| 159 | PROPOSED | [What a subshell copies, given that a capability set cannot always be copied](159-what-a-subshell-copies.md) |
| 160 | PROPOSED | [Which subset counts as running Vaultwarden](160-what-counts-as-running-vaultwarden.md) |
| 161 | PROPOSED | [Whether a holder can give up a mapping, and what gives it up](161-giving-up-a-mapping.md) |
| 162 | PROPOSED | [Where a confined device's IOMMU fault is delivered](162-where-a-device-fault-is-delivered.md) |
| 163 | PROPOSED | [Whether the kernel resolves a tid it already sent to the supervisor that received it](163-resolving-a-tid-a-supervisor-holds.md) |
| 164 | PROPOSED | [Where a stored secret comes from on a boot that is not a test](164-where-a-stored-secret-comes-from.md) |
| 165 | PROPOSED | [The rasteriser dependency, and whether the glyph atlas ships one face or four](165-the-rasteriser-and-how-many-faces.md) |
| 166 | PROPOSED | [What a profiling session's grant names](166-what-a-profiling-session-may-name.md) |
| 167 | PROPOSED | [Where a proof-gated credential rotation verb lives](167-where-a-rotation-verb-lives.md) |
| 168 | PROPOSED | [Whether a clipboard exists here, and what it is scoped to](168-whether-a-clipboard-exists.md) |
| 169 | PROPOSED | [How a foreign program is told what to do](169-how-a-foreign-program-is-told-what-to-do.md) |
| 170 | PROPOSED | [Where a program image starts, and where the stack goes](170-where-a-program-image-starts.md) |
| 171 | PROPOSED | [Whether a `credential_protocol` verify endpoint names the identity it asks about](171-credential-endpoint-per-resource.md) |
| 172 | PROPOSED | [Narrowing the root of the shell's namespace: a verb on the wire, or a shallower root](172-narrowing-the-namespace-root.md) |
| 173 | PROPOSED | [Which caller each of the three uncalled instruments gets](173-a-caller-for-the-three-uncalled-instruments.md) |
| 174 | PROPOSED | [Where the kernel's own output goes once userspace owns the console](174-kernel-console-arbitration.md) |
| 175 | PROPOSED | [Offering the two RedoxFS patches upstream, and under whose name](175-offering-the-redoxfs-patches-upstream.md) |
| 176 | PROPOSED | [Whether AGENTS.md quotes measured numbers at all](176-measured-numbers-in-the-front-door-file.md) |
| 177 | PROPOSED | [Where the timer re-arm seam goes, and which miss behaviour the kernel tick is meant to have](177-timer-rearm-seam.md) |
| 178 | PROPOSED | [Whether the tour boot keeps starting a console server that has no client](178-console-server-with-no-client.md) |
| 179 | PROPOSED | [Whether `components/` splits again, for the tools a person invokes](179-a-third-program-directory.md) |
| 180 | PROPOSED | [May `script/bootstrap` say "installed everything I could, and this machine is still not good enough" without failing?](180-bootstrap-provisions-and-judges.md) |
| 181 | PROPOSED | [Is a string two binaries agree on a name for `script/names`' purposes, or is it data?](181-provenance-for-wire-visible-names.md) |
| 182 | PROPOSED | [What `script/ci-build` with no arguments means, and what the two tiers are called](182-what-no-arguments-means.md) |
| 183 | PROPOSED | [Does the host test pass run on a second architecture, and at what cadence?](183-an-x86-64-host-in-the-host-pass.md) |
| 184 | PROPOSED | [What carries the claim that userspace composes a process from an authority you can count on one hand](184-composing-a-process-from-two-capabilities.md) |
| 185 | PROPOSED | [Where a riscv64 tour-boot check runs, what it asserts, and what it is called](185-where-the-riscv-tour-check-runs.md) |
| 186 | PROPOSED | [One crate per kernel-test pair, or one crate for all of them?](186-crates-for-the-numbers-a-kernel-test-and-its-program-agree-on.md) |
| 187 | PROPOSED | [What the lifted `fn check(ok: bool)` is called, now that nine programs write it out by hand](187-one-home-for-the-trap-on-false-helper.md) |
| 188 | PROPOSED | [Which of two definitions `caretaker` carries, and what the translating shape is called](188-what-a-caretaker-is-when-it-translates.md) |
| 189 | PROPOSED | [Must an icount baseline save record why it moved, and does a second fixed anchor earn its cost?](189-what-a-baseline-save-must-record.md) |
