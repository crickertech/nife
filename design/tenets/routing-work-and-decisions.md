...
proposed milestone or as a `BUGS` entry, an open decision lives in `design/decisions/`, and work held
for calef carries the `needs-architect` label and a `## What I need from you` comment. This file
carries why each of those exists and what failed without it. The watcher machinery is in
[`notes/merge-queue.md`](../../notes/merge-queue.md) and the sweep that found the category is in
[`notes/untracked-work-sweep.md`](../../notes/untracked-work-sweep.md). Moved here 2026-09-23 (UTC) on
