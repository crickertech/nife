...
// ---------------------------------------------------------------------------
// The redraw, folded in from `crates/watch` by milestone 281.
//
// **Name provenance, carried here rather than left in a deleted file.** This half of the crate was
// `crates/watch` until 2026-09-13. That name was **never ratified**: both it and the `watch` program
// shipped flagged provisional (milestone 126), on the argument that `watch` is what upstream
// `procps` calls the tool this is a narrower version of (`dpkg -L procps` lists `/usr/bin/watch`),
// which the naming tenet calls the best name available for a standard term a reader already knows.
// It was flagged anyway because this is genuinely narrower than upstream's: one fixed built-in view,
// not an arbitrary command line. Milestone 281 retired the name rather than ruling on it, which is
// the outcome the flag was there to keep open. The refusal that decided it: **two programs are two
// programs when they hold different authority**, and `ps` and `watch` held the same three slots from
// the same `grant_plan` constants, so a second crate for one program's presentation was a split with
// nothing behind it. `crates/watch` re-exported `MAX_ROWS`, `Row`, `Survey` and `collect` verbatim,
// which was the tell.
// ---------------------------------------------------------------------------
...
/// **The default and only interval today** (no `ArgSpec` register is spent on it; see
/// `user/src/watch.rs`'s module docs). Half a second: fast enough that a person watching a
/// short-lived domain sees it change, slow enough that the yield-spin loop this kernel's missing
/// timed wait forces on every interval user does not visibly burn a core the way `timetable`'s
/// un-throttled fire loop can.
...
/// (`user/src/swish.rs`); this bound keeps a plausible `watch` run (a domain of a few dozen threads) well
/// under that regardless of how the shell's own limit moves later. It is generous rather than tight:
/// a person who wants more than this many redraws of a static demo domain is better served by
...
/// **One redrawn frame of a [`collect`]ed table**: [`REDRAW`], then exactly what [`Survey::write_report`]
/// would have printed for the same walk, or (when there is nothing to print) the one-line reason a
/// person would otherwise be staring at a blank, unexplained screen for. `out` is called with a
/// `\n`-terminated sentence in that second case and never mixes the two: a survey either has rows to
/// show or it has a [`Survey::complaint`], never both, which is [`Survey`]'s own invariant and not
/// something this function checks twice.
pub fn frame(survey: &Survey<'_>, out: &mut dyn FnMut(&[u8])) {
    ...
    if let Some(clause) = survey.complaint() {
        ...
        // starts; see `user/src/watch.rs`), so the only complaint a frame can carry is the benign one:
...
mod tests {
    ...
    // -----------------------------------------------------------------------
    // The redraw's own tests, folded in from `crates/watch` by milestone 281.
    // -----------------------------------------------------------------------
