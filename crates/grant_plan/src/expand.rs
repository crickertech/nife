...
/// matches `filesystem_protocol::nameset::MAX_NAMES`, pinned by a host test.
///
...
/// a set, and a literal operand is simply the set of one. Same caretaker shape, same `filesystem_protocol`
...
/// the set itself (`filesystem_protocol::nameset`), and because a listing is a rendering of authority.
#[derive(Clone, Copy, PartialEq, Eq)]
pub struct NameSet {
