//! **Measured boot: the hash the kernel checks the progenitor against** (milestone 22 phase B.1).
//!
//! The kernel's promise is "a verified core that confines unverified workloads" (DECISIONS §14).
//! The progenitor is the unverified component that builds every other process, and until this crate existed
...
//! enter userspace on a mismatch. "This kernel runs exactly this progenitor," enforced.
//!
...
//! uses to notice stale build inputs) would let someone craft a colliding progenitor. Among the
...
/// just packed, which is what makes the check mean "this kernel runs exactly this progenitor" without any
...
/// in the archive it measures. The progenitor *is* in that archive, so its own table cannot be compiled into
...
///
/// The lookup happens **before** the hash, deliberately: the progenitor runs this over every program it is
...
/// to read as a noun at the call site. Public function and type names are an architect's (AGENTS.md,
...
mod tests {
    ...
    /// milestone rests on: an attacker who rewrites one byte of the progenitor's code cannot keep the
    /// measurement.
    #[test]
    ...
    /// text, and the progenitor decides what to load from it. So the same three verdicts have to hold when
