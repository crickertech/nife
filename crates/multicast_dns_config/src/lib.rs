...
//!
//! `multicast_dns_protocol` takes the whole advertisement as data and hard-codes none of it, deliberately: the
...
//! let config = multicast_dns_config::Config::parse(doc).expect("the document parses");
...
/// ([`multicast_dns_protocol::MAX_LABEL_LEN`]), applied at parse time so an over-long name is refused with a
/// line number rather than as an `Overflow` from the emitter with nothing to point at.
pub const MAX_LABEL: usize = multicast_dns_protocol::MAX_LABEL_LEN;
...
mod tests {
    ...
    const REFERENCE: &str = include_str!("../../../components/multicast_dns_responder.conf");
    ...
    /// **The document survives the round trip to the wire**: parsed, handed to `multicast_dns_protocol`, and
    ...
    fn the_disks_reach_the_adisk_txt_record_in_order() {
        ...
        let adv = c.advertisement(None);
        let mut out = [0u8; 512];
        let n = multicast_dns_protocol::adisk_txt_rdata(&adv, &mut out).unwrap();
        let txt = &out[..n];
