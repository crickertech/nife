...
//!
//! ```
//! use filesystem_protocol::{reply_err, reply_errno};
//!
...
//!
//! ```
//! use filesystem_protocol::{PAGE, fs};
//!
...
pub mod fs {
    ...
    ///
    /// Name: provisional (milestone 47 (navigation and naming)'s mtime lane, 2026-08-24); calef's
    /// to ratify.
    ...
    /// of the wall clock milestone 51 landed (`clock_protocol`, DECISIONS §43). A `touch` on two
...
pub mod dir {
    ...
    /// behaviors need two rights)); calef's to ratify. Candidates considered and set aside:
    /// `BACKDATE` (the capability also permits a *future* value, so a name for lying only backward
    /// would be narrower than what it grants), `RETIME` (read as "change the pace of something" as
    /// readily as "reassign its recorded time"). `SETTIME` was chosen for reading as exactly what
    /// it grants, at the cost of sitting one letter from [`super::fs::SETMTIME`]'s name; the two
    /// are always written module-qualified (`dir::SETTIME` against `fs::SETMTIME`), the same way
    /// [`CREATE`] already sits one identifier from [`super::fs::CREATE`] without confusion.
    ...
    mod proofs {
        ...
        /// Falsification: replayable `crates/filesystem_protocol/falsifications/dir.proofs.attenuate_never_widens.patch`
        #[kani::proof]
        ...
        /// Falsification: replayable `crates/filesystem_protocol/falsifications/dir.proofs.a_root_carries_nothing_undefined.patch`
        #[kani::proof]
...
pub mod fixture {
    ...
    ///
    /// Name: provisional (2026-08-18, milestone 38 (filesystem throughput)): calef names files a
    /// reader meets, and this one is met by anyone who lists the bench image after a run.
    ...
    pub mod throughput {
        /// **The transfer unit: the whole file channel, which is the most a single `filesystem_protocol`
    ...
    ///
    /// Name: provisional (2026-08-17, milestone 31 (a capability shell) phase 3): it names the one
    /// step that can fail before a caretaker serves anything, which is the single `OPENDIR` its
    /// whole attenuation lives in.
...
mod tests {
    ...
    /// stream with `byte_sink_protocol::eof()` and its status and count ride in the two words that message
    ...
    ///
    /// `script/swish-check` asserts two literal strings at a real prompt, and a literal is only
