...
pub struct BootEndowment {
    ...
    pub gpu_surface: u64,
    /// **A virtio-gpu, when this boot has one** (milestone 600 (provisional); milestone 177 built
    ...
    /// children, which is what a supervisor that swaps `display_terminal` live needs (milestone 23).
    ...
    pub gpu_irq: u64,
    /// The gpu's whole DMA region: one `PageFrame` capability naming the run (DECISIONS §102), rings
