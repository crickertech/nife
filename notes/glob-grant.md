...
Same caretaker shape, same `filesystem_protocol` protocol above and below, nothing new in the kernel.
...
reached a name the pattern never matched. It is now `filesystem_protocol::verb`'s `takes_name()`, one row per
...
name is zero bytes long (`filesystem_protocol::grant::WHOLE_NAMESPACE`). A name cannot be empty, so that
...
The batching, on the host too (`cargo test -p grant_plan -p swish -p fs_proto`, milestone 109 (`xargs`)).
...
- The set filter and the FS server read the name twice, out of one page, and only the first read
  is checked (milestone 43 (a second security audit)'s audit, notes/shared-page-audit.md findings 1 and 2). The caretaker
  ...
  becomes live once the progenitor builds a *set* caretaker for the prompt, which
  [a-set-grant-at-the-prompt.md](a-set-grant-at-the-prompt.md) proposes. A frame per client channel is the fix and is proposed as its own milestone.
...
- A sweep cannot run anything that takes a directory grant yet, which is the same missing
  ...
  stops at the first one, because every batch is a set and the progenitor builds no set caretaker. What is proven at a real
...
- At a real prompt a pattern may designate one name, and a set is refused. Corrected
  2026-09-26: two entries here said the prompt holds no directory and `spawn` refuses every
  directory grant. Since milestone 31 (a capability shell) phase 3 (2026-08-17) the boot shell holds the file service
  and the progenitor builds a `fs_subtree_caretaker` for one name one level down. Two or more names
  are refused in `dir_grant` (`components/src/swish.rs`), because `spawnproto` cannot carry a set.
  Only the kernel test wires a set caretaker. How a set should travel is PROPOSED in
  [a-set-grant-at-the-prompt.md](a-set-grant-at-the-prompt.md).
...
let report = fs_service::start_granted_set(
    ...
    fs_service::SetGrant {
        ...
        role: filesystem_protocol::grant::spec(0, 0),         // no name: the operand is the namespace
        arg: 0,
        arg2: 0,
        stack_pages: 4,
    },
