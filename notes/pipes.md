...
(`supervision_protocol` plus a `build_child` in each init) to `supervision_protocol`'s, which the inits
...
an architect's call", and calef called it: option (c), the manifest declaration.*
...
#### Why this is an architect's call
...
shell"](#the-file-behind-a--is-this-shell-and-that-was-not-the-plan) already gives: `filesystem_protocol` shares
...
It drives `helpers/qemu-runner-aarch64.sh` directly rather than `cargo run`, so the process it owns **is**
