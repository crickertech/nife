...
The heap is capped at 32 KiB (`swish::HEAP_MAX_BYTES`). It maps from a 12-page region
(`swish::HEAP_REGION_PAGES`) split off the shell's 128-page budget first thing at `_start`. That
region sits under every later carve, so it cannot break their last-in first-out return. The cap is
