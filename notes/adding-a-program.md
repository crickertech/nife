...
**The name is an architect's** (AGENTS.md, "calef names the crates, the programs, and the shared modules").
...
own author called wrong is the shortest conversation an architect can have. This page told newcomers the
...
- **Every archive packs every program, including ones that cannot run there**, which is the rule
  ...
  rule prices it at.
- **Whether a program may take an argument and an input together is open**, and it is an architect's call:
