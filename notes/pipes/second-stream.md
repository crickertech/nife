...
The analysis of the fork as it stood open is kept in [its own appendix](second-stream-fork.md), because the reasoning is the reusable part
...
[The section on
`script/swish-check`](tests-and-the-boot-gate.md#the-gate-for-the-boot-itself-which-is-what-runs-the-real-progenitor) says it "is the only thing in the tree that runs the real
