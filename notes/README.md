...
- [The works this project is arguing with](bibliography.md): every outside work some page in this
  tree actually reads, with what was taken from it, under one rule that keeps it honest: a work
  somebody means to read is not an entry. Split out of `README.md` on 2026-09-21, and it carries the
...
- [The multi-tasking workload benchmark](job-mix.md): milestone 168's instrument, the one
  ...
  Since 2026-09-13 it also carries the premise check, read out of Warton's own thesis rather than
  the retrospective: his AIM7 ran on Wombat, a hosted Linux, over two tasks with the disk and
  network jobs disabled, and he doubted the 20% himself. No number from this instrument is
  comparable with it; the mechanism rather than the number is what the instrument can find.
...
- [What rented metal costs](rented-metal.md): the priced comparison §203 (capacity is rented rather
  ...
  riscv64 at €0.042/hr; **no aarch64 offer qualifies today**; every free tier fails on boot or on
  cores. Equinix Metal shut down on 30 June 2026 and Hetzner's Arm line is unorderable. Also: what a
  second provider per architecture is an experiment for, what multi-vendor actually costs, and why
  milestone 225 (run the soak on radon, argon and xenon)'s x86_64 leg is under a euro of machine
  time. Name provisional.
