...
- **No hardware TRNG yet, but "needs verifying" is now precise rather than open-ended** (milestone
  ...
  silicon. `components/src/jh7110_trng.rs` is a full `entropy_proto` backend built on that logic, over a
  ...
  **Wired on 2026-09-01**: `entropy_service`'s `Bus` enum grew a `Jh7110` variant and the riscv64
  ...
  touches a register; see `components/src/jh7110_trng.rs`'s `BUGS`), and the whole question below.
