...
Milestone 47. Built 2026-07-31. The contract side is `filesystem_protocol::fs::RMDIR`; the program is
...
nothing, and `filesystem_protocol` has a host test asserting it is never staged.
...
name is **zero bytes long** (`filesystem_protocol::grant::WHOLE_NAMESPACE`). It means "the operand is your
...
- **`rm` removes nothing this contract has since grown**, and that is now checked rather than
  assumed. Milestone 61 made the caretakers dispatch off `filesystem_protocol::verb`, so a verb added to the
