...
| `script/job-mix [--arch A] [--smp N]` | Rehearse milestone 168's multi-tasking workload sweep under QEMU: boot `--features jobmix`, where the tour ends in a pool of EL0 tasks swept through a heterogeneous job mix instead of `arch::halt()`, echo the sweep, and tear the emulator down when it prints `jobmix: done`. Exit statuses 0 the sweep completed, 1 the kernel refused to start it, 3 QEMU ended early, 4 build or arguments. **This is the rehearsal and not the measurement**: the milestone's gate is HARDWARE, TCG models no cache, and the number `design/decisions/96-process-kernel-or-event-kernel.md` is waiting for is taken on radon by notes/job-mix.md's procedure. No `--check` and no committed baseline, deliberately: a sweep whose subject is scheduling under contention is not deterministic on any accelerator this tree has. **Name provisional.** See notes/job-mix.md. |
