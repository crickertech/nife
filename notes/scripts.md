...
trade. `setup`/`update` do the heavy dependency work; `test` stays fast; `ci-build` provisions on its
no-argument path, because that is the command a person runs on a cold checkout before pushing. A
named check (`ci-build fmt`) does not, because the caller naming one check is a CI job that has
already installed exactly what it needs, and a rustfmt runner has no business apt-installing QEMU.
