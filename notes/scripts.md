...
| `script/board-netboot` | Serve `target/board` over TFTP so radon boots without a card (milestone 257): read-only, one directory, `blksize` and `tsize`, udp/69 without `sudo`. Prints the `setenv nife_boot_server` line for every private address this machine has, which is the other half of the boot script's echoed expectation. `--root <dir>`, `--port <n>`. Written here rather than delegated to dnsmasq because dnsmasq is a DHCP server that also does TFTP, and this bench LAN is a house network with a router already handing out leases; the reason is in the script's own header. **Name provisional.** See notes/visionfive2.md. |
