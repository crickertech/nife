...
| `script/roadmap` | Index the milestones; `--check` validates the status vocabulary and catches a block with no row, or a milestone cited in prose the table does not carry. Also gates milestone 247's `## Follow-on` section on every finished block, so the work a milestone named on its way out cannot go nowhere. `--ready` lists what a lane could start today, which is where work a lane identifies now goes: it writes the numbered block itself, `NOT-STARTED`, with the number provisional (calef, 2026-09-19). `--outstanding` lists what each PARTIAL milestone still says is left (milestone 252). `--unclaimed` and `--proposed` were retired by milestone 434 with the `design/roadmap/proposals/` directory they read. Gated in `lint`. |
...
| `script/job-mix [--arch A] [--smp N]` | Rehearse milestone 168's multi-tasking workload sweep under QEMU: boot `--features job_mix`, where the tour ends in a pool of EL0 tasks swept through a heterogeneous job mix instead of `arch::halt()`, echo the sweep, and tear the emulator down when it prints `job-mix: done`. Exit statuses 0 the sweep completed, 1 the kernel refused to start it, 3 QEMU ended early, 4 build or arguments. **This is the rehearsal and not the measurement**: the milestone's gate is HARDWARE, TCG models no cache, and the number `design/decisions/96-process-kernel-or-event-kernel.md` is waiting for is taken on radon by notes/job-mix.md's procedure. No `--check` and no committed baseline, deliberately: a sweep whose subject is scheduling under contention is not deterministic on any accelerator this tree has. **Name provisional.** See notes/job-mix.md. |
