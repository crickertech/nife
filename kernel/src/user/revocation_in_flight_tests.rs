...
//! DECISIONS §13's use-after-free reopened through the one slot the sweep does not look in.
//!
...
fn a_capability_revoked_while_it_is_in_flight_does_not_reach_the_receiver() {
    ...
    // Clock-bounded rather than yield-counted: since DECISIONS §28 (placement scatters threads
    // across cores) the sender is on another core and this core's yields say nothing about it.
    assert!(
        super::wait_for(
