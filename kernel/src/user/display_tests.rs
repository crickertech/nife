...
///
/// The pattern is a per-coordinate function ([`graphics_protocol::pixel`]), not a fill, and the digest is
/// position sensitive, so a blank, stale, shifted, transposed, or truncated surface cannot pass
/// (`crates/graphics_protocol`'s host tests assert exactly those properties of the pattern itself). Two
...
/// `screendump` (which works headlessly), and compares the PPM against `graphics_protocol::pixel` pixel
