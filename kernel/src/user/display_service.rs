...
///
/// **The region is one contiguous run, and has been since milestone 142** (DECISIONS §102): one
/// `alloc_contiguous_zeroed` of [`DMA_PAGE_FRAMES`] frames, registered with `crate::virtio` as one
/// region, and granted as one `PageFrame` capability. That is the fact milestone 600 (provisional)
/// was promoted to check, because milestone 177 had built the graphical stack kernel-side on the
/// premise that the driver needed a capability per DMA page.
