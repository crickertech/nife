...
/// What `Spawn::arg0` means. Must match `components/src/entropy.rs`; the kernel's own spawn-argument
...
/// Where the service maps the TRNG's register page. **Must match `components/src/jh7110_trng.rs`'s
...
/// not an `Irq` capability, because the driver polls (`components/src/jh7110_trng.rs` records why);
