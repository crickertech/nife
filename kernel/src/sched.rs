...
/// reply capabilities are swept anyway**, which is `cteDeleteOne(callerCap)` from `cancelIPC`,
/// because the thread name is generational and a `Reply` that outlives its thread would go stale
/// only on its *next use*: a server holding one is holding a capability whose target this function
/// just ended, and leaving it in place is leaving the tree to depend on a slot generation nobody
/// reading `ipc_reply` can see. Cost is O(threads x slots) on a teardown path.
///
...
fn finish_blocked_resident(sched: &mut IpcTables, tid: ThreadId) {
    ...
    // Sweep the outstanding reply capabilities. `delete_matching` is the same primitive the
    // `PageFrame` reclamation sweeps use; this cannot go through `delete_page_frame_caps_where`
    // because that takes `IPC_TABLES`, which the caller already holds.
    let stale = crate::cap::Object::Reply(tid);
    for t in sched.threads.iter_mut() {
        t.capability_table
            .delete_matching(|object| *object == stale);
    }
